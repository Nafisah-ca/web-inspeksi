    ...
    <div class="card p-6 text-left mb-6">
        ...
        <dl class="space-y-3 text-sm">
            <div class="flex justify-between">
                ...
                <dd class="font-mono font-semibold text-gray-900">{{ $booking->booking_code }}</dd>
            </div>
            <div class="flex justify-between">
                <dt class="text-gray-500">Paket</dt>
                <dd class="font-medium text-gray-900">{{ $booking->package->name }}</dd>
            </div>
            <div class="flex justify-between">
                <dt class="text-gray-500">Kendaraan</dt>
                <dd class="font-medium text-gray-900">{{ $booking->vehicle->brand }} {{ $booking->vehicle->model }}</dd>
            </div>
            <div class="flex justify-between">
                <dt class="text-gray-500">No. Plat</dt>
                <dd class="font-medium text-gray-900">{{ $booking->vehicle->plate_number }}</dd>
            </div>
            <hr>
            <div class="flex justify-between">
                <dt class="text-gray-500">Tanggal</dt>
                <dd class="font-semibold text-gray-900">{{ $booking->booking_date->format('d M Y') }}</dd>
            </div>
            <div class="flex justify-between">
                <dt class="text-gray-500">Waktu</dt>
                <dd class="font-semibold text-gray-900">{{ substr($booking->booking_time, 0, 5) }} WIB</dd>
            </div>
            <hr>
            <div class="flex justify-between">
                <dt class="text-gray-500">Total</dt>
                <dd class="font-bold text-lg text-blue-600">{{ $booking->package->formatted_price }}</dd>
            </div>
        </dl>
    </div>
