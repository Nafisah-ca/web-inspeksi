        ...
        <div class="grid lg:grid-cols-2 gap-12 items-center">
            <div>
                ...
                <h1 class="text-4xl lg:text-5xl font-extrabold leading-tight mb-6">
                    {{ $hero['title_line1'] ?? 'Inspeksi Kendaraan' }}<br>
                    <span class="text-blue-200">{{ $hero['title_line2'] ?? 'Profesional & Transparan' }}</span>
                </h1>
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    <a href="{{ route('booking.create') }}"
                       class="inline-flex items-center justify-center gap-2 bg-white text-blue-700 font-bold px-7 py-3 rounded-xl hover:bg-blue-50 transition-colors shadow-lg">
                        ...
                        {{ $hero['btn_primary'] ?? 'Booking Sekarang' }}
                    </a>
                    <a href="{{ route('packages') }}"
                       class="inline-flex items-center justify-center gap-2 border-2 border-white/50 text-white font-semibold px-7 py-3 rounded-xl hover:bg-white/10 transition-colors">
                        {{ $hero['btn_secondary'] ?? 'Lihat Paket' }}
                    </a>
                </div>
            </div>
            ...
            <div class="hidden lg:flex justify-center">
                <div class="relative">
                    ...
                    <div class="absolute -top-4 -right-8 bg-white rounded-2xl shadow-xl p-4 min-w-[160px]">
                        <p class="text-xs text-gray-500 mb-1">{{ $stats['stat1_label'] ?? 'Inspeksi Selesai' }}</p>
                        <p class="text-2xl font-bold text-gray-900">{{ $stats['stat1_value'] ?? '0' }}+</p>
                    </div>
                    <div class="absolute -bottom-4 -left-8 bg-white rounded-2xl shadow-xl p-4 min-w-[160px]">
                        <p class="text-xs text-gray-500 mb-1">{{ $stats['stat3_label'] ?? 'Inspektor Terlatih' }}</p>
                        <p class="text-2xl font-bold text-gray-900">{{ $stats['stat3_value'] ?? '0' }}+</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 md:grid-cols-4 divide-x divide-gray-100">
            ...
            <div class="py-6 px-4 text-center">
                <p class="text-2xl font-bold text-gray-900">{{ $icons[$i-1] }} {{ $val }}</p>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid md:grid-cols-3 gap-6">
            ...
            <div class="card p-6 hover:shadow-md transition-shadow relative {{ $i === 1 ? 'border-blue-500 border-2 ring-4 ring-blue-50' : '' }}">
                ...
                @endif
                <div class="mb-4">
                    <div class="w-10 h-10 rounded-xl mb-3 flex items-center justify-center {{ ['bg-green-100 text-green-600','bg-blue-100 text-blue-600','bg-purple-100 text-purple-600'][$i] ?? 'bg-gray-100 text-gray-600' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<section class="py-16 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12">
            <h2 class="text-3xl font-bold text-gray-900 mb-3">{{ $how['title'] }}</h2>
            @if(!empty($how['subtitle']))
            <p class="text-gray-500">{{ $how['subtitle'] }}</p>
            @endif
        </div>
        ...
        <div class="grid md:grid-cols-4 gap-6">
            ...
            @endphp
            @for($s = 1; $s <= 4; $s++)
            @if(!empty($how["step{$s}_title"]))
            <div class="text-center">
                <div class="inline-flex items-center justify-center w-14 h-14 {{ $stepColors[$s-1] }} text-white text-xl font-bold rounded-2xl mb-4">
                    {{ $s }}
                </div>
                <h3 class="font-semibold text-gray-900 mb-2">{{ $how["step{$s}_title"] }}</h3>
                @if(!empty($how["step{$s}_desc"]))
                <p class="text-sm text-gray-500 leading-relaxed">{{ $how["step{$s}_desc"] }}</p>
                @endif
            </div>
            @endif
            @endfor
...
<section class="py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-10">
            <h2 class="text-3xl font-bold text-gray-900">{{ $why_us['title'] }}</h2>
        </div>
        <div class="grid md:grid-cols-3 gap-8">
            @for($w = 1; $w <= 3; $w++)
            @if(!empty($why_us["item{$w}_title"]))
            <div class="text-center">
                <div class="text-4xl mb-3">{{ $why_us["item{$w}_icon"] ?? '✅' }}</div>
                <h3 class="font-semibold text-gray-900 mb-2">{{ $why_us["item{$w}_title"] }}</h3>
                @if(!empty($why_us["item{$w}_desc"]))
                <p class="text-sm text-gray-500 leading-relaxed">{{ $why_us["item{$w}_desc"] }}</p>
                @endif
            </div>
            @endif
            @endfor
...
<section class="py-16 bg-blue-600">
    <div class="max-w-3xl mx-auto px-4 text-center">
        <h2 class="text-3xl font-bold text-white mb-4">{{ $cta['title'] }}</h2>
        @if(!empty($cta['subtitle']))
        <p class="text-blue-100 mb-8">{{ $cta['subtitle'] }}</p>
        @endif
        <a href="{{ route('booking.create') }}"
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <a href="tel:{{ preg_replace('/[^0-9+]/', '', $contact['phone']) }}"
               class="card p-5 hover:shadow-md transition-shadow text-center group">
                ...
                <p class="text-sm font-semibold text-gray-900 group-hover:text-blue-600">{{ $contact['phone'] }}</p>
            </a>
            @endif
            ...
            <a href="mailto:{{ $contact['email'] }}"
               class="card p-5 hover:shadow-md transition-shadow text-center group">
                ...
                <p class="text-sm font-semibold text-gray-900 group-hover:text-blue-600">{{ $contact['email'] }}</p>
            </a>
            @endif
            ...
            <div class="card p-5 text-center">
                ...
                <p class="text-sm font-semibold text-gray-900">{{ $contact['address'] }}</p>
            </div>
            @endif
            @if(!empty($contact['open_hours']))
            <div class="card p-5 text-center">
                <div class="text-3xl mb-2">🕐</div>
                <p class="text-xs text-gray-400 font-semibold uppercase mb-1">Jam Buka</p>
                <p class="text-sm font-semibold text-gray-900">{{ $contact['open_hours'] }}</p>
            </div>
            @endif
        </div>
