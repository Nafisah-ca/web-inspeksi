...
@section('page-title', 'Hasil Inspeksi')

@section('content')

<div class="flex items-center justify-between mb-5 flex-wrap gap-3">
    <p class="text-sm text-gray-500">
        Total <span class="font-semibold text-gray-900">{{ $results->total() }}</span> laporan inspeksi
        @if(request()->hasAny(['search','date']))
            <a href="{{ route('admin.results.index') }}" class="ml-2 text-blue-600 hover:underline text-xs">(reset filter)</a>
        @endif
    </p>
</div>
...
<div class="card p-4 mb-5">
    <form method="GET" class="flex flex-wrap gap-3 items-end">
        ...
        <div class="flex gap-2">
            ...
            @if(request()->hasAny(['search','date']))
            <a href="{{ route('admin.results.index') }}" class="btn-secondary btn-sm h-9">Reset</a>
            @endif
        </div>
    </form>
</div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                @php $booking = $result->booking; @endphp
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="table-td text-sm">
                        <p class="text-gray-900">{{ $booking->vehicle->brand }} {{ $booking->vehicle->model }}</p>
                        <p class="text-xs font-mono text-gray-400">{{ $booking->vehicle->plate_number }}</p>
                    </td>
